...
import { getBoundaryLayer, addFloatLayer } from './other.js';
...
// 测面积图形的样式
const measureAreaStyle = new ol.style.Style({
  fill: new ol.style.Fill({
    color: [34, 142, 255, 0.15],
  }),
  stroke: new ol.style.Stroke({
    color: [34, 142, 255, 1], // 蓝色
    width: 1
  })
});
...
const _defaultSetting = {
  defaultMapCenter: from4326To3857([102.44, 28.19]), // 地图默认中心点--凉山
  lngField: 'lng', // 坐标经度的字段名
  latField: 'lat', // 坐标纬度的字段名
  isFitToAllGeos: true, // 是否根据定位点适配可视范围
  // 是否仅首次加载定位点时适配可视范围
  ...
  selectMap: false, // 地图切换控件
  // 其他
  isShowBoundary: false, // 是否显示区域边界
  hasFloatLayer: false // 是否显示地图浮层
}
...
Fence.prototype = {
  ...
  setRadius(radius) {
    isNumber(radius) && this.feature.getGeometry().setRadius(radius);
  },
  getRadius() {
...
function handlerSelected(event){
  ...
  function setOffset(infoWindowType, str, overlay) {
    str = str || '';
    var len = str.length,
  ...
  features = features.filter(item => !!item.__type); // 过滤掉非本组件定义所属类型的feature
  if (features.length && !this.drawing) {
    hasTarget = true;
    ...
    var _type = feature.__type,
      infoWindowType;
    if (_type == 'geoPoint' && this.infoWindows.geoPointInfo) {
      this.showInfoWindow(feature.__geoPoint, 'geoPointInfo');
    }
    if (_type == 'marker' && this.infoWindows.markerInfo) {
      infoWindowType = feature.__marker.data.isSaved ? 'markerInfo' : 'markerEdit';
      setOffset(infoWindowType, feature.__marker.data.remark, this.infoWindows.markerInfo.overlay);
      this.showInfoWindow(feature.__marker, infoWindowType);
    }
    // 避免判断成线
    if (_type == 'fence' && !this.drawing
      && feature.__fence.type != 'LineString'
      && this.infoWindows.fenceInfo) {
      if(feature.__fence.modifyListener) return;
      infoWindowType = feature.__fence.data.isSaved ? 'fenceInfo' : 'fenceEdit';
      setOffset(infoWindowType, feature.__fence.data.remarks, this.infoWindows.fenceInfo.overlay);
    ...
    if (_type && isFunction(this.selectedCallbacks[_type])) {
...
function MapTool(options) {
  ...
  this.boundaryLayer = getBoundaryLayer(ol, this.opts.isShowBoundary);
...
MapTool.prototype = {
  init(mapContainer, sourceName) {
    ...
    this.map = new ol.Map({
      ...
      view: new ol.View({
        ...
        extent: (() => {
          if(isArray(this.opts.extent) && this.opts.extent.length == 4){
    ...
    // 添加浮层
    this.opts.hasFloatLayer === true && addFloatLayer(this.map);
  },
  getDoubleClickZoom() {
    var me = this;
    forEach(me.map.getInteractions().getArray(), function(item, i) {
  ...
  // projectionLink传入coordinate的EPSG
  setCenter(coordinate, projectionLink){
    if(isArray(coordinate) && coordinate.length == 2){
      if(projectionLink !== 'EPSG:3857'){
        coordinate = from4326To3857(coordinate);
      }
  ...
  createDrawInteraction(type) {
    ...
    function handle_drawend(event) {
      ...
      this.sketch.setStyle(fence.type == 'LineString' ? measureLineStyle : measureAreaStyle);
  ...
  loadGeos(arr, type) {
    ...
    arr.forEach((item, i, array) => {
      ...
      item.posCoordinateSystem = item.coordinate || 0;
    ...
    if (!this.opts.isOnlyFirstFitToAllGeos ||
      ...
      this.opts.isFitToAllGeos && this.fitToAllGeos();
    }
  },
  // 加载标注点
  loadMarkers(arr) {
  ...
  removeFence(id) {
    if (this.fences[id]) {
      if (isArray(this.fences[id].modifyListeners)) {
      ...
      var dbclickHandler = function(event) {
        this.map.forEachFeatureAtPixel(event.pixel, (function(feature) {
          if (feature.__fence && feature.__fence.id == target.id) {
            ...
            isFunction(success) && success(target, old_coords, old_radius);
          }
        }).bind(this));
      }
